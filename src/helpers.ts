...
export const dropBucket = (data: Pixel[], dropIndex: number, color: string, initialColor: string, initialData: Pixel[]) => {
  ...
  if (bottomIndex < PIXEL_COUNT * PIXEL_COUNT && newData[bottomIndex].color === initialColor && newData[bottomIndex].color !== color) {
    newData = dropBucket(newData, bottomIndex, color, initialColor, initialData);
  }

  if ((leftIndex + 1) % PIXEL_COUNT !== 0 && newData[leftIndex].color === initialColor) {
    newData = dropBucket(newData, leftIndex, color, initialColor, initialData);
  }

  if (rightIndex % PIXEL_COUNT !== 0 && newData[rightIndex].color === initialColor && newData[rightIndex].color !== color) {
